...
> 当前版本：**v2.2.1** | 最低 AstrBot 版本：**>=4.24.2**
...
- **防刷屏检测（v2.2.1）**：追踪所有消息类型（转发/QQ收藏/图片/JSON等）的发送频率，支持每秒/每分钟/每小时速率限制，超限自动禁言+可选撤回，撤回阈值可自定义
...
| **审核规则热更新**            | 支持通过 WebUI 动态添加/修改/删除正则规则和词库关键词，修改即时生效无需重启插件                                                                                             | v2.5.0 |
...
| **Aho-Corasick 自动机匹配** | 词库关键词已全面替换为 AC 自动机（`KeywordAutomaton`），脏话/广告规则通过 `HybridMatcher` 统一匹配，长文本审核性能提升百倍以上                                               | 已完成 |
