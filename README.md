...
> 当前版本：**v2.2.4** | 最低 AstrBot 版本：**>=4.24.2**
...
| 功能 | 说明 | 目标版本 |
|------|------|---------|
| 审核规则热更新 | WebUI 直接增删改规则和关键词，即时生效 | v2.3.0 |
| 入群自动审核 | 处理加群请求，自动通过/拒绝 | v2.3.0 |
| 群名片/昵称违规检测 | 改名含违规词自动拦截 | v2.5.0 |
| 语音消息审核 | 接入语音转文字后送 LLM 审核 | v2.5.0 |
| 群文件审核 | 拦截含违规内容的文件名或文件内容 | v2.5.0 |
| 链接安全检测 | 检查消息中的 URL 是否涉诈/钓鱼/赌博 | v2.4.0 |
| GIF 帧级拆分审核 | 动图逐帧送视觉模型，不漏中间帧违规 | v2.5.0 |

### 处罚机制
| 功能 | 说明 | 目标版本 |
|------|------|---------|
| 违规积分累进制 | 首次警告 → 二次禁言 → 三次踢出 | v2.4.0 |
| 禁言前私聊通知 | 先 DM 警告再动手，给申诉机会 | v2.3.0 |
| 定时自动解禁 | 到预设时间自动解封 | v2.3.0 |
...
| 功能 | 说明 | 目标版本 |
|------|------|---------|
| 长文本刷屏 | 单条消息超 N 字判定违规 | v2.3.0 |
| 重复消息检测 | 同内容发多次自动处置 | v2.3.0 |
| 外链邀请撤回 | 检测外部群邀请链接并撤回 | v2.4.0 |
| 凌晨时段独立限速 | 夜间与白天可配置不同的速率上限 | v2.4.0 |

### 权限与配置
| 功能 | 说明 | 目标版本 |
|------|------|---------|
| 单用户白名单 | 指定用户完全豁免所有审核 | v2.3.0 |
| 多群独立配置 | 每个群各自设置审核策略/词库/时长 | v3.0.0 |
