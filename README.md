...
> 当前版本：**v1.9.10** | 最低 AstrBot 版本：**>=4.24.2**
...
| **代码多文件架构拆分**        | 将当前单文件 main.py（3000+行）按职责拆分为多个独立模块：审核引擎（moderation.py）、Web API（web_api.py）、群管命令（commands.py）、QQ收藏检测（qq_favorite.py）、工具函数（utils.py）等，提升代码可维护性和可读性，便于社区贡献和功能扩展                                                                                                  | v2.0.0 |
...
| **多群独立配置**              | 每个群可独立配置审核策略、词库开关、禁言时长、通知模板等，不同群使用不同规则互不干扰；WebUI 支持按群切换配置视图                                                                                                                                           | v2.1.0 |
| **Aho-Corasick 自动机匹配** | 将词库关键词匹配从正则引擎替换为 Aho-Corasick 自动机算法（flashtext），在长文本审核中性能提升百倍以上，内存占用降低90%                                                                                                                            | v2.2.0 |
| **多协议适配**              | 适配 Telegram/Discord 等其他平台的群管功能                                                                                                                                                                      | v3.0.0 |
...
3. 本产品在本地存储的审核日志（`moderation_logs.json`）和配置文件由使用者自行管理，使用者应采取合理措施保护这些文件中的敏感信息不被未授权访问。
